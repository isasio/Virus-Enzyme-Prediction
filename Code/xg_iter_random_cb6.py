...
for i in range(1, 501):
    # Set an unique random seed for each iteration
    random.seed(i)
    np.random.seed(i)
    
    # Set the output file names for each run
    file_output = f'xg_correct_predictions_class_20seq_20_{i}.csv'
    history_output = f'xg_training_history_iter_20seq_20_{i}.csv'
    ...
    for iteration in range(iterations):
        ...
        classifier = xgb.XGBClassifier(n_estimators=20)
